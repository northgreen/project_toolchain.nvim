...
local function open_history(mode, callback)
    if callback ~= nil then -- async
        path.create_datadir(function(_, _)
...
--- remove duplicates dir from a table (will normalise path before comparison)
--- @param tbl table
--- @return table
local function delete_dir_duplicates(tbl)
	local normalise_path = path.normal_path
    local cache_dict = {}
...
function M.delete_project(project)
    for k, v in ipairs(M.recent_projects) do
        if v == project.value then M.recent_projects[k] = nil end
    end
end

--- Deserialize history data from string to table
...
local function deserialize_history(history_data)
    ...
    projects = delete_dir_duplicates(projects)
...
local function setup_watch()
    ...
    if M.has_watch_setup == false then
        ...
        event:start(path.datapath, {}, function(err, _, events)
...
local function sanitize_projects()
    ...
    tbl = delete_dir_duplicates(tbl)

    local real_tbl = {}
    for _, dir in ipairs(tbl) do
        if dir_exists(dir) then table.insert(real_tbl, dir) end
    end
...
function M.write_projects_to_history()
    ...
    if file ~= nil then
        ...
        local out = ""
        for _, v in ipairs(tbl_out) do out = out .. v .. "\n" end
